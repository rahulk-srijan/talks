	...
	$.extend($.autopager, {
		...
		load: function() {
			if (active || !nextUrl || options.disabled) {
                ...
                /**Not working on cross domain changed by ranjeet@srijan.in start**/
                       if(nextUrl){
                            var ex_href = nextUrl.split("/");
                            if(ex_href[1]){
                                //var t = 'globalcomm';
                                var str_length = parseInt(ex_href[1].length+1);
                                if(ex_href[1]== ex_href[2]){
                                    nextUrl = nextUrl.substr(str_length,9000);

                                }
                            }
                       }
            /**Not working on cross domain changed by ranjeet@srijan.in end**/
			active = true;
